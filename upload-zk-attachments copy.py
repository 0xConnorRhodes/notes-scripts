...
# TODO: handle links in the format ![[../zattachments/file.png]]
...
for dir in attachment_dirs:
    ...
    for file in attachment_files:
        ...
        local_file_links = []
        local_file_links.append(f"![[{local_file}]]")
        local_file_links.append(f"![[../zattachments/{local_file}]]")
        local_file_links.append(f"![[zattachments/{local_file}]]")

        # rg_command = f"rg -l -F '{local_file_link}' {notes_dir}"
        # rg_command = f"rg -l -F '!\[\[.*{local_file}]]' {notes_dir}"

        parent_files = []
        for link in local_file_links:
            rg_command = f"rg -l -F '{link}' {notes_dir}"
        ...
        for link in local_file_links:
            replace_attachment_links(parent_files, link, embed_link)
